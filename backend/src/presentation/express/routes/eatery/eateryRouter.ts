...
import RegisterEateryApplicationService, {
  RegisterEateryCommand,
} from '../../../../application/eatery/registerEateryApplicationService/RegisterEateryApplicationService';
import GetEateryApplicationService from '../../../../application/eatery/getEateryApplicationService/GetEateryApplicationService';
import DeleteEateryApplicationService, {
  DeleteEateryCommand,
} from '../../../../application/eatery/deleteEateryApplicationService/DeleteEateryApplicationService';
...
eateryRouter.get('/', async (req, res) => {
  try {
    const getEateryApplicationService = new GetEateryApplicationService(
      repository,
    );
...
eateryRouter.post('/', upload.array('eateryImages', 2), async (req, res) => {
  try {
    ...
    const eateryRegularHolidays = JSON.parse(
      eateryRegularHolidaysParsed,
    ) as string[];
    let imageUrl: string[] = [];
...
eateryRouter.delete('/:eateryId', async (req, res) => {
  try {
    const { eateryId } = req.params;
    console.log('eateryId@delete-eateryRouter', eateryId);
    const deleteEateryApplicationService = new DeleteEateryApplicationService(
      repository,
    );
    const deleteEateryCommand: DeleteEateryCommand = {
      eateryId,
    };
    console.log('deleteEateryCommand@delete-eateryRouter', deleteEateryCommand);
    await deleteEateryApplicationService.execute(deleteEateryCommand);

    res.status(200).json({ message: 'DELETE /eatery' });
  } catch (error) {
    res.status(400).json({ message: (error as Error).message });
  }
});
